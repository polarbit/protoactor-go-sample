...
func StartWorker(cPort int, sPort int, hosts []string) {
	// Setup actor system
	...
	remoteConfig := remote.Configure("127.0.0.1", sPort)
	...
	cp := automanaged.NewWithConfig(1*time.Second, cPort, hosts...)
	...
	signal.Notify(finish, syscall.SIGTERM, syscall.SIGINT)
	<-finish
}
